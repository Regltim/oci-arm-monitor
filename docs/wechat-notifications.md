...
5. 在“模板消息接口”中分别新增下面两个模板。旧版七字段模板不兼容，升级时也必须重新创建。
...
两个模板的字段名和顺序都必须保持为 `first`、`item1`、`item2`、`item3`。不要添加 `level`、`metric`、`status`、`content`、`time`、`remark`，否则收到的卡片会缺少内容，或被微信拒绝。
...
1. 在测试公众号或正式公众号中按本文内容新建“OCI ARM Monitor 运行通知”模板。
