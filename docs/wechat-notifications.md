...
每日摘要可以附带免登录 H5 明细链接。微信卡片仍只显示四行摘要，点击后查看发送时生成的脱敏快照，不需要登录监控后台。即时告警、恢复通知和手动模板测试不附带明细链接。
...
每日汇总默认关闭。开启后，系统在所选时区每天到达指定时间后发送一张运行摘要和一张费用流量摘要，不再追加补充卡片。两种模板独立去重和记录；运行信息发送失败不会阻止费用与流量消息。投递结果中的成功、失败数量按“摘要卡片数 × 接收人数”的实际接口请求次数统计。日报使用现有本地数据，不会额外触发 OCI 同步。

启用免登录明细时，每张日报在发送前写入不可变快照，每个接收人生成不同的随机令牌。令牌只出现在 URL Fragment 中，并通过 `Authorization` 请求公开快照接口；Nginx/OpenResty 的普通访问日志和 Referer 不会收到令牌。SQLite 只保存 SHA-256 哈希，错误、过期或撤销的令牌统一返回“报告不存在或已过期”。过期快照和令牌每天自动清理。
...
如果点击每日摘要后显示“报告不可用”，确认服务器时间正确、`MONITOR_PUBLIC_URL` 与实际 HTTPS 域名一致，并检查令牌是否超过设置的有效期。手动“测试两个模板”只验证公众号模板，不生成日报快照，因此测试消息没有 H5 跳转。
